...
DEP_FILE_NAME="lambda-contact-ses"
DEP_AWS_REGION="eu-west-1"
DEP_NAME="contact-response"
...
echo "  build       Run build script to build the code directory and terraform deploy to deploy to AWS infrastructure\n"
echo "  "
echo "FLAGS"
...
if [ -z "${CMD}" ]; then 
    CMD=help
elif [ ! ${CMD} =  "init" -a ! "${CMD}" = "plan" -a ! "${CMD}" = "validate" -a ! "${CMD}" = "deploy" -a ! "${CMD}" = "upload" -a ! "${CMD}" = "clean" -a ! "${CMD}" = "destroy"  -a ! "${CMD}" = "import" -a ! "${CMD}" = "build" ]; then
    CMD=help
fi

if [ "${CMD}" = "init" -o "${CMD}" = "validate" -o "${CMD}" = "plan" -o "${CMD}" = "deploy" -o "${CMD}" = "destroy" -o "${CMD}" = "upload" -o "${CMD}" = "clean" ]; then
